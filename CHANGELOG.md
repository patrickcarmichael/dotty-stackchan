...
### Changed
- **GPU bits split out of `docker-compose.yml`** (#10) — `runtime: nvidia`, the `deploy.resources` reservation block, and `NVIDIA_*` env vars now live in a new `compose.gpu.yml` override. The base compose is CPU-portable. `make setup` auto-detects the NVIDIA Container Toolkit via `docker info` and writes `COMPOSE_FILE=docker-compose.yml:compose.gpu.yml` into `.env` when present; on CPU hosts it picks `FunASR` instead of `WhisperLocal` so the stack comes up without CUDA. `make doctor` flags WhisperLocal+cuda mismatches against the detected runtime.
- **Setup wizard is now idempotent and doesn't dirty git** (#11) — `.config.yaml`, `docker-compose.yml`, and `zeroclaw-bridge.service` are now generated by `make setup` from `*.template` (tracked) using answers persisted in `.wizard.env`. All three live files and `.wizard.env` and `models/` are gitignored. Re-running `make setup` re-prompts with previous answers as defaults; pressing enter keeps each one. `make regen-config` re-renders from the saved answers without prompting. `server.timezone_offset` is now derived from the IANA TZ name via `date +%:z` (was hard-coded `+10`). Existing live files are backed up to `<file>.bak.YYYYMMDD-HHMMSS` before any divergent rewrite. The wizard now lives in `scripts/setup-wizard.sh`.
